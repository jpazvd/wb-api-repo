...
def _req(url, retries=4, backoff=0.8):
    ...
    for i in range(retries):
        try:
            r = requests.get(url, params={"format":"json"}, timeout=60); r.raise_for_status()
            j = r.json()
            if isinstance(j,list) and len(j)>=2: return j[1]
...
def main():
    if not os.path.exists(CFG): raise SystemExit(f"Config not found: {CFG}")
    with open(CFG,"r",encoding="utf-8") as f: cfg = yaml.safe_load(f) or {}
    jobs = cfg.get("jobs") or []
    if not jobs: 
        print("No jobs in config.yaml"); return
    for job in jobs:
        name = job.get("name") or "unnamed"
        inds = job.get("indicators"); countries = job.get("countries","all")
        date = job.get("date"); out = job.get("out"); long_flag = job.get("long", False)
        if not inds or not out:
            print(f"Skipping job {name}: indicators/out required"); continue
        v, inv = validate_indicators(inds)
        if inv:
            print(f"[WARN] Skipping job {name}: invalid indicator codes: {', '.join(inv)}"); continue
        os.makedirs(os.path.dirname(os.path.join(ROOT, out)), exist_ok=True)
        args = [PY, WB, "data", "--indicators", ",".join(v), "--countries", str(countries)]
        if date: args += ["--date", str(date)]
        if long_flag: args += ["--long"]
