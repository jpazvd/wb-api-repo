...
program define yaml_write
    ...
    * Open file for writing — replace mode only when the user asked for it.
    * The check above already errors out if the file exists without `replace`,
    * so a missing file here means the caller didn't pass replace; either way,
    * mode follows the user's intent rather than being unconditional.
    tempname fh
    if ("`replace'" != "") {
        file open `fh' using "`using'", write text replace
    }
    else {
        file open `fh' using "`using'", write text
    }
    ...
    * If scalars option, write scalars
    if ("`scalars'" != "") {
        foreach s of local scalars {
            * Confirm scalar exists. `capture scalar `s'` is wrong here —
            * it evaluates / displays the scalar (and may error) rather than
            * checking existence; `capture confirm scalar `s'` is the correct
            * idiom and is side-effect-free.
